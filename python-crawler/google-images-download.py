...
#### Initial code from hardikvasa/google-images-download
#### Usage
...
while i < len(search_keyword):
    ...
    ## creates a folder for each search term
    # dir_name = search_term + ('-' + args.color if args.color else '')
    ## creates one Image folder to store all images
    dir_name = 'All_Image'
    ...
    while (k < limit):
        try:
            ...
            if ".jpg" in image_name or ".png" in image_name or ".jpeg" in image_name or ".svg" in image_name:
                output_file = open(dir_name + "/" + search_keyword[i] + " " + str(k + 1) + ". " + image_name, 'wb')
            else:
                output_file = open(dir_name + "/" + search_keyword[i] + " " + str(k + 1) + ". " + image_name + ".jpg", 'wb')
                image_name = search_keyword[i] + " " + image_name + ".jpg"
